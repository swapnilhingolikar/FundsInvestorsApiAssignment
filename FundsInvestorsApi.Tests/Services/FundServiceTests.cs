...
using TransactionType = FundsInvestorsApi.Models.TransactionType;

namespace FundsInvestorsApi.Tests.Services
{
    /// <summary>
    /// Unit tests for FundService using mocked repository, AutoMapper, and logger.
    /// Covers CRUD operations and transaction summary functionality.
    /// </summary>
    public class FundServiceTests
    {
        ...
        {
            // Arrange: create mock summary DTOs (as the service expects repo method to return DTOs)
            var fund1Id = Guid.NewGuid();
            var fund2Id = Guid.NewGuid();

            var summaryFromRepo = new List<FundTransactionSummaryDto>
            {
                new FundTransactionSummaryDto
                {
                    FundId = fund1Id,
                    FundName = "Fund A",
                    TotalSubscribed = 100,
            ...
            _repoMock.Setup(r => r.GetTransactionSummaryAsync()).ReturnsAsync(summaryFromRepo);

            // Act
            var result = await _service.GetTransactionSummaryAsync();

            // Assert
            Assert.Contains(result, s => s.FundId == fund1Id && s.TotalSubscribed == 100 && s.TotalRedeemed == 50);
            Assert.Contains(result, s => s.FundId == fund2Id && s.TotalSubscribed == 200 && s.TotalRedeemed == 80);

            _repoMock.Verify(r => r.GetTransactionSummaryAsync(), Times.Once);
        }
